...
function ActivityDetails({ opened }) {
  ...
  const save = () => {
    dispatch(updateInfo(activity.id, newTitle, newDescription));
    setEditing(false);
  };

  const handleImageDelete = (image_id) => {
    dispatch(deleteImage(activity.id, image_id));
                ...
                >
                  {editing && (
                    <svg
                      ...
                      className={styles.details_img_delete}
                      width="28"
                      height="28"
                      viewBox="0 0 10 10"
                      fill="none"
